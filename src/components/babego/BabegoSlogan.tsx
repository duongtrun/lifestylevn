...
export default function BabegoSlogan() {
  return (
    <section className="w-full bg-[#FAFAFA] py-16 md:py-24 flex justify-center items-center overflow-hidden">
      <div className="relative w-full max-w-[1000px] px-4 flex justify-center items-center min-h-[300px] md:min-h-[400px]">
        
        {/* Khung viền vẽ tay (Oval) */}
        <div className="absolute inset-0 w-[90%] md:w-[85%] lg:w-[80%] h-full z-0 pointer-events-none mx-auto opacity-100">
          <Image 
            src="/img_babego/bg_slogan.svg" 
            alt="Babego Background Outline" 
            fill 
        ...
        {/* Mascot nằm chèn lên khung bên phải */}
        <motion.div 
          initial={{ opacity: 0, x: 20 }}
          whileInView={{ opacity: 1, x: 70 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.2 }}
          className="absolute right-0 md:right-[-5%] lg:right-[-2%] top-1/2 -translate-y-1/2 w-[180px] h-[240px] md:w-[310px] md:h-[400px] lg:w-[410px] lg:h-[500px] z-20 pointer-events-none"
        >
          <Image 
        ...
        <motion.div 
          ...
          className="relative z-10 flex flex-col items-center text-center px-8 sm:px-12 md:px-28 py-12 md:py-16 mt-4 w-full md:w-[85%] lg:w-[80%]"
        >
          {/* Logo Babego Gold */}
          ...
          {/* Tiêu đề */}
          <h2 className="text-sm sm:text-base md:text-[20px] font-bold text-[#444444] mb-3 md:mb-5">
            Babego Gold – Tiêu hóa vững vàng, tăng cân dễ dàng
          </h2>

          {/* Lời hứa */}
          <p className="text-[11px] sm:text-sm md:text-[15px] text-[#444444] max-w-[280px] sm:max-w-[400px] md:max-w-[650px] leading-relaxed md:leading-loose">
